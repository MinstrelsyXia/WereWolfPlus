...
def parse_args():
    ...
    # parser.add_argument("--has_sheriff", required=False, default=True,help="whether to add the role sheriff in werewolf")
    parser.add_argument("--disable_wandb", required=False, default=True, help="whether to disable wandb")
...
class TaskMultiEval(object):
    def __init__(self, comm_args, task):
        ...
        # 设置默认值
        self.args.setdefault('has_sheriff', False)
        ...
        # 禁用wandb
        if self.comm_args.disable_wandb:
            os.environ['WANDB_MODE'] = 'disabled'
            try:
    ...
    def calc_eval_ret(self, ret_all,store=True):
        if not self.comm_args.disable_wandb:
            ...
            if self.args.game.game_name.__contains__("WereWolf"):
                ...
                wandb.log(task_result)
                self.logger.info(f"player_win_rate: {task_result['player_win_rate']}, match_turns_avg: {task_result['match_turns_avg']}")
...
if __name__ == '__main__':
    ...
    warnings.filterwarnings("ignore")
    import wandb
    from tasks_config import TASKS, WANDB_API_KEY, WANDB_TIMEOUT, WEAVE_OPEN
    if WEAVE_OPEN:
        import weave
    import yaml
    from box import Box
    ...
    # 添加freeze_support()以解决Windows多进程错误
